...
/**
 * Tests for SimpleContact
 *
 * @package SimpleContact
 * @copyright Center for History and New Media, 2007-2014
 */
class SimpleContact_Test extends Omeka_Test_AppTestCase {
    ...
    {
        parent::setUp();
        $integrationHelper = new SimpleContact_IntegrationHelper;
    ...
    public function testSendMailToContacter() {
        ...
        $this->assertThat($mailText, $this->stringContains('Subject: ' . get_option('site_title') . " - " . get_option('simple_contact_notification_user_subject')));
        $this->assertThat($mailText, $this->stringContains(get_option('simple_contact_notification_user_header')));
    }
    ...
    public function testSendMailToAdministrator() {
        ...
        $this->assertThat($mailText, $this->stringContains('Subject: ' . get_option('site_title') . " - " . get_option('simple_contact_notification_admin_subject')));
        $this->assertThat($mailText, $this->stringContains(get_option('simple_contact_notification_admin_header')));
    }

    public function testContactFormRedirect() {
        $this->assertRedirectTo('/' . SIMPLE_CONTACT_PATH . '/thankyou');
    }
}
